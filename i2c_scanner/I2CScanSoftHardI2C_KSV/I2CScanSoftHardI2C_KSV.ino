...
//KSV: пробуем работать с цифровыми портами. Работает!
/*
  so instead of specifying the number of the 
  digital pin (0-19) the port (PORTB, PORTC, PORTD) 
  and the port pin has to be specified.    
...
void loop(void)
{
  uint8_t add = 0;
  int found = false;
  byte i = 0x5A; //termometer MLX90614ESF

   Wire.begin();
   Wire.beginTransmission (i);
    if (Wire.endTransmission () == 0)
      {
      Serial.print ("Wired Found address: ");
      Serial.print (i, DEC);
      Serial.print (" (0x");
      Serial.print (i, HEX);
      Serial.println (")");
