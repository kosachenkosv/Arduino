/*HiTechnic Infrared Seeker library for Arduino
  By B. Blechschmidt | August 1, 2013 |
...
Today I have created a class allowing you to connect a HiTechnic IR Seeker V2, which is originally designed for use with a LEGO Mindstorms NXT robot, to an Arduino Due board with a 9V battery. I struggled a bit with the the different sensor pins but I could finally manage to connect the sensor correctly.
    смотреть на разъем со стороны контактов
    
    white -> battery +9V (Vcc) (в самодельном разъеме красный=белый +9в)
    black -> Arduino GND
    red -> battery – (GND)
    green -> Arduino +3.3V (Vcc)
...
InfraredResult InfraredSeeker::ReadAC()  //читает модулированный ИК 1200Гц
{
  byte buffer[6];
  ReadACRaw(buffer);
...
InfraredResult InfraredSeeker::ReadDC() //читает немодулированный ИК солнце
{
  byte buffer[6];
  ReadDCRaw(buffer);
  return PopulateValues(buffer);
...
void loop()
{   
  InfraredResult InfraredBall = InfraredSeeker::ReadAC(); //читает модулированный ИК 1200Гц
